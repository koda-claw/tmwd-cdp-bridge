...
Tag releases with `vX.Y.Z` after updating `Cargo.toml`, README/API examples if needed, `docs/releases/vX.Y.Z.md`, and the extension version when extension files change.
...
Every packaged release must include `docs/releases/vX.Y.Z.md`. The release
workflow uses that file as the GitHub Release body so published packages do not
ship with only an autogenerated changelog.
